...
func TestSession_InitSession(t *testing.T) {
	...
	if sessKind != reflect.ValueOf(sm).Kind() {
		t.Error("wrong kind returned while testing a cookie session. Expected", reflect.ValueOf(sm).Kind(), "and got instead", sessKind)
	}

	if sessType != reflect.ValueOf(sm).Type() {
		t.Error("wrong type returned while testing a cookie session. Expected", reflect.ValueOf(sm).Type(), "and got instead", sessType)
	}
}
